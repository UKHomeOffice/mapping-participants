...
module.exports = {
  ...
  getData: (token, tokenSecret, surveyID) => {
    const client = new SmartSurveyClient({ apiToken: token, apiTokenSecret: tokenSecret})
    ...
    //   console.log(result)
    // })
    client.getResponses(surveyID, { page: 1, pageSize: 25, includeLabels: true}, function (err, result) {
    if (err) {
      console.log(err)
    }
    else {
